...
🔟 Тема 10 [источник]"""
    ...
    def generate_post(self, theme: str, technique: str = "cov+cok", post_length: int = 500) -> tuple:
        ...
        input_text = f"""Сгенерируй пост на тему "{theme}" с применением данных из File Search и Web Search с применением Chain of Knowledge и перепроверкой фактов cov+cok
...
1️⃣ ЗАГОЛОВОК (СТРОГО ПЕРВАЯ СТРОКА ПОСТА):
   ...
   • Пример: 💪 **ГРЕБЛЯ CONCEPT2: КАЛОРИИ ГОРЯТ КАК НИКОГДА**
   • Пример: 🧠 **СИТАПЫ: БЕЗОПАСНАЯ ТЕХНИКА ДЛЯ ПРЕССА**
   • ⚠️ Заголовок ОБЯЗАТЕЛЕН и должен соответствовать теме "{theme}"!

2️⃣ ЛИД (ЗАТРАВКА - 1-3 ПРЕДЛОЖЕНИЯ ПОСЛЕ ЗАГОЛОВКА):
   ...
   • Пример: "Часто слышу: 'Зачем долго разминаться? Я устану!'"
   • Пример: "'После тренировки я чувствую себя в 10 раз лучше!' Это не плацебо."
   • ⚠️ Лид ОБЯЗАТЕЛЕН - он должен ЗАЦЕПИТЬ и заставить читать дальше!
...
🔥 КРИТИЧЕСКИ ВАЖНО:
- ⚠️ БЕЗ ЗАГОЛОВКА И ЛИДА = НЕПРАВИЛЬНЫЙ ПОСТ!
- Заголовки секций ОБЯЗАТЕЛЬНО в ** (например: 🔥 **НАЗВАНИЕ СЕКЦИИ:**)
- Каждый факт с источником (ВОЗ)/(PubMed)/(CrossFit)/(Исследования)
- Без маркера ">" в начале (не цитата!)

НАЧИНАЙ ПОСТ С ЗАГОЛОВКА В CAPS!"""
